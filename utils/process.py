...
def process2corpus():
    ...
    unk_id = 0
    with open(DoulistFile) as fopen, open(DoulistCorpusNameFile, 'w',encoding='utf-8') as fwrite, open(DoulistCorpusIdFile, 'w') as fwrite_1:
        for line in fopen:
            doulist_dict = json.loads(line.strip())
            ...
            fwrite.write('%s\n' % ('\t'.join([str(_.decode()) for _ in doulist_movies])))
